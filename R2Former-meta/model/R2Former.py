...
class R2Former(nn.Module):
    def __init__(self, img_size=(480,640), decoder_embed_dim=384, decoder_depth=4, decoder_num_heads=6,
    ...
    def forward(self, x_global, x_rerank=None, y_global=None, y_rerank=None): #x表示查询，y表示正样本或者负样本
        ...
        x_rerank_token = F.normalize(x_rerank[:, :, 3:], p=2, dim=2) #查询图片的局部特征
        y_rerank_token = F.normalize(y_rerank[:, :, 3:], p=2, dim=2) #候选图片的局部特征
        x_coordinate = x_rerank[:, :, :3].detach().clamp(min=0, max=1)  #截取前最后一维的前3个，xy坐标和patch索引
        y_coordinate = y_rerank[:, :, :3].detach().clamp(min=0, max=1)

        correlation = torch.matmul(x_rerank_token, y_rerank_token.permute((0, 2, 1)))#做自注意力，得到每个patch对所有patch的注意力值，基于xy坐标和全匹配模型的自注意力值（注意是局部特征）
        xy_matrix = torch.cat(
            [x_coordinate.unsqueeze(2).repeat(1, 1, y_rerank_token.shape[1], 1), #【B，top-k-patch，top-k-patch，【（x，y），att】】
             y_coordinate.unsqueeze(1).repeat(1, x_rerank_token.shape[1], 1, 1), correlation.unsqueeze(3)],#【B，top-k-patch，top-k-patch，【（x，y），att】】 ，【B，top-k-patch，top-k-patch，1】
            dim=3)
        order_q = torch.argsort(correlation.unsqueeze(3), dim=2, descending=True).repeat(1, 1, 1, 7) #查询x对候选y的注意力值排序
        order_k = torch.argsort(correlation.unsqueeze(3), dim=1, descending=True).repeat(1, 1, 1, 7) #候选y对查询x的注意力值排序
        select_q = torch.gather(input=xy_matrix, index=order_q[:, :, :self.num_corr, :], dim=2) #筛选查询x对候选y的num_corr个最高注意patch，并从特征融合后的xy_matrix中选取出对应部分
        select_k = torch.gather(input=xy_matrix, index=order_k[:, :self.num_corr, :, :], dim=1) #筛选num_corr个候选y对查询x的最高注意patch，并从特征融合后的xy_matrix中选取出对应部分
        ...
        x = torch.cat([self.cls_token_2.repeat(B*N_select, 1, 1), pair_matrix], dim=1)#将局部特征加入一个提取块，输入transformer交互提取信息到cls_token_2
