...
def collate_fn(batch):
    ...
    for i, (local_indexes, global_indexes) in enumerate(zip(triplets_local_indexes, triplets_global_indexes)):#zip是迭代器，enumerate添加索引i
        local_indexes += len(global_indexes) * i  # Increment local indexes by offset (len(global_indexes) is 12)，整合后的图像在本地索引中，要加入对应的增量12才是新的本地索引位置
...
class TripletsDataset(BaseDataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train", negs_num_per_query=10):
        ...
        if self.mining == "msls_weighted":
            ...
            logging.info(f"#sideways_indexes [{len(sideways_indexes)}/{self.queries_num}]; " +
                         f"#night_indexes[{len(night_indexes)}/{self.queries_num}]")
    ...
    def compute_triplets_partial(self, args, model):
        self.triplets_global_indexes = []
        # Take 1000 random queries
        print(self.queries_num, args.cache_refresh_rate)
