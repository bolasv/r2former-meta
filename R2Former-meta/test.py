...
def test_rerank(args, eval_ds, model, test_method="hard_resize", pca=None, num_local=196, rerank_dim=256, rerank_top=20, rerank_bs=4, save=None, reg_top=5,ransac=False, threshold=0, debug=False):
    """Compute features of the given dataset and compute the recalls."""
    ...
    if args.dataset_name == 'msls':
        predictions_str = []
        for query_index, pred in enumerate(predictions):
            string = [os.path.basename(eval_ds.queries_paths[query_index]).replace('.jpg','')]
            for pred_id in pred:
                string.append(os.path.basename(eval_ds.database_paths[pred_id]).replace('.jpg',''))
            predictions_str.append(string)
        predictions_str = np.array(predictions_str)
        if eval_ds.split == 'test':
            create_dummy_predictions(prediction_path=os.path.join(args.save_dir, 'global.csv'), dataset=eval_ds, ranks=predictions_str[:,1:200])
