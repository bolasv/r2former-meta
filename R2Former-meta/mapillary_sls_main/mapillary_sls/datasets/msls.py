...
'''
default_cities = {
    'train': ["amman","nairobi","boston","goa"],
    'val': ["sf"],
    'test': ["kampala"]
}
class MSLS(Dataset):
    def __init__(self, root_dir, save=False, cities = '', nNeg = 10, transform = None, mode = 'train', task = 'im2im', subtask = 'all', seq_length = 1, posDistThr = 5, negDistThr = 10, cached_queries = 1000, cached_negatives = 1000, positive_sampling = True):
        ...
        for city in self.cities:
            ...
            if self.mode in ['train','val']:
                ...
                qSeqKeys, qSeqIdxs = self.arange_as_seq(qData, join(root_dir, subdir, city, 'query'), seq_length_q)  #单图对单图不需要，所以这里的arange_as_seq并没有实际操作，因为帧序列长度为1
                ...
                if self.mode in ['val']:
                    ...
                    val_frames = np.where(qIdx[self.subtask])[0] #按照任务列是否为true返回行索引
                ...
                seqIdx2frameIdx = lambda seqIdx, seqIdxs : seqIdxs[seqIdx] #：根据 “序列索引” 获取该序列包含的所有帧索引
                frameIdx2seqIdx = lambda frameIdx, seqIdxs: np.where(seqIdxs == frameIdx)[0][1] #根据 “帧索引” 查找该帧所在的序列索引和在序列中的位置。
                frameIdx2uniqFrameIdx = lambda frameIdx, uniqFrameIdx : np.where(np.in1d(uniqFrameIdx, frameIdx))[0]    #将 “帧索引” 转换为它在 “唯一帧索引数组” 中的位置索引
                uniqFrameIdx2seqIdx = lambda frameIdxs, seqIdxs : np.where(np.in1d(seqIdxs,frameIdxs).reshape(seqIdxs.shape))[0] #根据 “一组唯一帧索引”，查找包含这些帧的所有序列的索引
                ...
                # D：距离数组，每个元素是一个列表，包含查询样本与对应数据库正样本的距离。
                # I：索引数组，每个元素是一个列表，包含查询样本对应的数据库正样本的索引（即 “哪些数据库样本与该查询是同一位置”）
        ...
        self.nonNegIdx_hard = np.asarray(self.nonNegIdx_hard, dtype=object)
    ...
    def arange_as_seq(self, data, path, seq_length):
        '''按照序列长度，将路径中的连续图像分段整合作为新的单元，
        例子：数据中包含一个视频的所有帧，总共50帧，按照序列长度10进行截取
        输入：data（使用他的索引），path（需要分段的数据路径），seq_length（序列长度）
        返回：seq_keys【】字符串数组，一个序列长度的key集合  seqidxs 序列组对应索引'''
    ...
    def filter(self, seqKeys, seqIdxs, center_frame_condition):
        '''按照中心帧是否在子任务序列帧中进行添加，若序列长度为1，则为判断每个元素是否符合子任务'''
        keys, idxs = [], []
